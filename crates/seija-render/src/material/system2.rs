...
use bevy_ecs::{prelude::{World, Added}, event::{ManualEventReader, Events}, entity::Entity, change_detection::Mut};
use seija_asset::{AssetEvent, Handle, Assets, HandleId};
use smol_str::SmolStr;
use wgpu::CommandEncoder;
use crate::{pipeline::render_bindings::{BindGroupLayoutBuilder, BindGroupBuilder}, resource::{RenderResources, BufferId}, memory::align_num_to};
use super::{MaterialDefineAsset, MaterialDef, Material};
...
impl MaterialSystem {
    ...
    pub fn get_buffer_layout(&self) -> &wgpu::BindGroupLayout { &self.common_buffer_layout }
    ...
    pub fn update(&mut self,world:&mut World,res:&mut RenderResources,commands: &mut CommandEncoder) {
        ...
        world.resource_scope(|w,mut materials:Mut<Assets<Material>>| {
            let mut added_materials = w.query_filtered::<(Entity,&Handle<Material>),Added<Handle<Material>>>();
            let mut query = w.query::<(Entity,&Handle<Material>)>();
            //add buffer
            for (e,hmat) in added_materials.iter(w) {
                let h_mat:&Handle<Material> = hmat;
                if let Some(mat) = materials.get(&h_mat.id) {
            ...
            for (e,h_mat) in query.iter(w) {
                ...
                if let Some(mat) = materials.get_mut(&h_mat.id) {
                    if let Some(define) = self.datas.get_mut(mat.def.name.as_str()) {
                        ...
                        let did = if mat.props.is_dirty() { Some(h_mat.id.clone()) } else { None };
                    ...
                    if let Some(dirty_id) = item.dirty_hid.as_ref() {
                        if let Some(mat) = materials.get(dirty_id) {
                            if cur_has_dirty == false {
                                ...
                                res.write_mapped_buffer(&define.cache_buffer, start..(start + buffer.len() as u64), &mut |bytes,_| {
                        ...
                        if cur_has_dirty {
                            ...
                            res.copy_buffer_to_buffer(commands,
                                &define.cache_buffer, 
                                0, 
                                &define.gpu_buffer,
                            0,
...
impl MaterialDefine {
    pub fn new(define:Arc<MaterialDef>,res:&mut RenderResources) -> Self {
        let buffer_item_size:u64 = align_num_to(define.prop_def.size() as u64, wgpu::BIND_BUFFER_ALIGNMENT);
        ...
        MaterialDefine {
            buffer_item_size, 
            define,texture_layout,
            cache_buffer,gpu_buffer,
            cap:default_cap,len:0,
    ...
    pub fn update_buffer(&mut self,id:u32,res:&mut RenderResources,layout:&wgpu::BindGroupLayout,material:&mut Material,dirty_hid:Option<HandleId>) {
