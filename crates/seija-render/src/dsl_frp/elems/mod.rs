...
impl IElement for ElementNode {
    ...
    fn active(&mut self,world:&mut World,ctx:&mut RenderContext,_:&mut FRPSystem) -> Result<()> {
       self.node.active(world, ctx)
    }

    fn deactive(&mut self,world:&mut World,ctx:&mut RenderContext,_:&mut FRPSystem) -> Result<()> {
...
impl IElement for TextureElement {
    ...
    fn deactive(&mut self,_:&mut World,_:&mut RenderContext,frp_sys:&mut FRPSystem) -> Result<()> {
        if let Some(dynamic) = frp_sys.dynamics.get_mut(&self.dyn_texture) {
            if let Some(dynamic_ptr) = dynamic.value.cast_userdata() {
                unsafe { let _ = Box::from_raw(dynamic_ptr as *mut RenderResourceId); };
            }
            dynamic.value = Variable::Nil
        }
        Ok(())
    }
}