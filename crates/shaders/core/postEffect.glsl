...
vec4 fxaa_fs_main(TexVSOutput o) {
  return vec4(1);//fxaa_console(o.uv,tex_texture0,tex_texture0Sampler);
}
...
vec4 fs_fragBlur(VSUV5 i) {
    float weight[3] = { 0.4026, 0.2442, 0.0545 };
		vec3 sum = texture(sampler2D(tex_texture0,tex_texture0Sampler), i.uv[0].xy).rgb * weight[0];
    
		for (int it = 1; it < 3; it++) {
			sum +=  texture(sampler2D(tex_texture0,tex_texture0Sampler), i.uv[it * 2 - 1].xy).rgb * weight[it];
			sum +=  texture(sampler2D(tex_texture0,tex_texture0Sampler), i.uv[it * 2].xy).rgb * weight[it];
		}
