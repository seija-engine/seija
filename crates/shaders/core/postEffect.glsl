...
vec4 colorByOffset(float x,float y,vec2 texelSize,vec2 uv) {
  return texture(sampler2D(tex_mainTexture,tex_mainTextureSampler),uv  + vec2(x, y) * texelSize);
}
...
vec4 fxaa_fs_main(TexVSOutput o) {
  ...
  if(isHorizontal) {
    pixelStep = vec2(0, texelSize.y);
  } else {
    pixelStep = vec2(texelSize.x, 0);
  }
