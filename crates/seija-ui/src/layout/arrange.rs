...
fn arrange_flex_element_nowrap(entity: Entity,flex: &FlexLayout,element: &LayoutElement,parent_origin: Vec2,parent_size: Vec2,params: &LayoutParams) -> Vec2 {
    ...
    match flex.justify {
        ...
        //分散对齐,两边贴靠
        FlexJustify::SpaceBetween => { arrange_flex_spacebetween(entity,lt_pos,flex,params) }
        //分散对齐，两边是中间的一半
...
fn arrange_flex_spacebetween(entity: Entity,lt_pos:Vec2,flex:&FlexLayout,params: &LayoutParams) {
    let flex_iter = FlexIter::new(params.childrens.get(entity).ok(), flex.direction);
...
fn arrange_flex_element_wrap(entity: Entity,flex: &FlexLayout,element: &LayoutElement,parent_origin: Vec2,parent_size: Vec2,params: &LayoutParams) -> Vec2 {
    Vec2::ZERO
}
