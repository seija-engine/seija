...
impl ImageGenericInfo {
    pub fn build_mesh(&self,mat:&Mat4,rect2d:&Rect2D,uv:Rect<f32>,atlas_index:usize,raw_size:&Rect<u32>,z_order:f32) -> Mesh2D {
        match &self.typ {
            ImageType::Simple => { self.build_simple_mesh(mat,rect2d,uv,atlas_index,z_order) },
            ImageType::Sliced(thinkness) => { self.build_slice_mesh(thinkness,mat,rect2d,uv,atlas_index,raw_size,z_order) },
            _ => {
                todo!()
            }
        }
    ...
    pub fn build_simple_mesh(&self,mat:&Mat4,rect2d:&Rect2D,uv:Rect<f32>,atlas_index:usize,z_order:f32) -> Mesh2D {
        ...
        let points = vec![
              Vertex2D { //left top
                 ...
                 uv:  [uv.x,uv.y].into(),
                 uv1:Vec2::new(atlas_index as f32, 0f32)
              },
              Vertex2D { //right top
    ...
    pub fn build_slice_mesh(&self,thickness:&Thickness,mat:&Mat4,rect2d:&Rect2D,uv:Rect<f32>,atlas_index:usize,raw_size:&Rect<u32>,z_order:f32) -> Mesh2D {
        ...
        //center
        ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x1,x2,y1,y2,uv_x1,uv_x2,uv_y1,uv_y2, z_order,atlas_index);
        ImageGenericInfo::fill_quad_index(&mut indexs, ia);
        //left top
        if thickness.left > 0f32 && thickness.top > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x0,x1,y0,y1,uv_x0,uv_x1,uv_y0,uv_y1, z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs, ia);
        }
        //top
        if thickness.top > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x1,x2,y0,y1,uv_x1,uv_x2,uv_y0,uv_y1,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs, ia);
        }
        //right top
        if thickness.right > 0f32 && thickness.top > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x2,x3,y0,y1,uv_x2,uv_x3,uv_y0,uv_y1,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
        //left
        if thickness.left > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x0,x1,y1,y2,uv_x0,uv_x1,uv_y1,uv_y2,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
        //right
        if thickness.right > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x2,x3,y1,y2,uv_x2,uv_x3,uv_y1,uv_y2,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
        //bottom left
        if thickness.bottom > 0f32 && thickness.left > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x0,x1,y2,y3,uv_x0,uv_x1,uv_y2,uv_y3,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
        //bottom
        if thickness.bottom > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x1,x2,y2,y3,uv_x1,uv_x2,uv_y2,uv_y3,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
        //bottom right
        if thickness.right > 0f32 && thickness.bottom > 0f32 {
            ImageGenericInfo::fill_quad_mesh(&mut meshes,mat,x2,x3,y2,y3,uv_x2,uv_x3,uv_y2,uv_y3,z_order,atlas_index);
            ia += 4;
            ImageGenericInfo::fill_quad_index(&mut indexs,ia);
        }
    ...
    pub fn fill_quad_mesh(verts:&mut Vec<Vertex2D>,mat:&Mat4,l:f32,r:f32,t:f32,b:f32,uv_l:f32,uv_r:f32,uv_t:f32,uv_b:f32,z:f32,atlas_index:usize) {
        //left top
        verts.push(Vertex2D { pos: mat.mul_vec4(Vec4::new(l, t, z, 1f32)).xyz(), uv: Vec2::new(uv_l, uv_t),uv1:Vec2::new(atlas_index as f32, 0f32) });
        //right top
        verts.push(Vertex2D { pos: mat.mul_vec4(Vec4::new(r, t, z, 1f32)).xyz(), uv: Vec2::new(uv_r, uv_t),uv1:Vec2::new(atlas_index as f32, 0f32) });
        //left bottom
        verts.push(Vertex2D { pos: mat.mul_vec4(Vec4::new(l, b, z, 1f32)).xyz(), uv: Vec2::new(uv_l, uv_b),uv1:Vec2::new(atlas_index as f32, 0f32) });
        //right bottom
        verts.push(Vertex2D { pos: mat.mul_vec4(Vec4::new(r, b, z, 1f32)).xyz(), uv: Vec2::new(uv_r, uv_b),uv1:Vec2::new(atlas_index as f32, 0f32) });
    }
